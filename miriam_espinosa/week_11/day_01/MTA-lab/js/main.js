...
lineInElm.addEventListener('change', function(e){
  ...
  populateStops(subwaySystem[lineIn], stopInElm)
});

submit.addEventListener('click', function(e){
  e.preventDefault();
  let lineIn = lineInElm.options[lineInElm.selectedIndex].value;
  let stopIn = stopInElm.options[stopInElm.selectedIndex].value;
  let lineOut = lineOutElm.options[lineOutElm.selectedIndex].value;
  let stopOut = stopOutElm.options[stopOutElm.selectedIndex].value;
  fullTrip(lineIn, stopIn, lineOut, stopOut);
})

const populateStops = function(stops, elm) {
  elm.innerHTML = "";
  stops.forEach(function (stop,i) {
    let option = document.createElement('option');
    option.setAttribute('value', stop)
    option.innerHTML = stop;
    elm.appendChild(option)
  })
}
populateStops(subwaySystem['nLine'], stopInElm);
populateStops(subwaySystem['sixLine'], stopOutElm);

lineOutElm.addEventListener('change', function(e){
  let lineOut = lineOutElm.options[lineOutElm.selectedIndex].value;
  populateStops(subwaySystem[lineOut], stopOutElm)
})
